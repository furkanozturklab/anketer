...
export const scrollToSection = async (section, setOldAnketer) => {
  await setOldAnketer(true);

  document.querySelectorAll(section)[0].style.cssText = "display:flex";
  document.querySelectorAll(section)[0].scrollIntoView({
    behavior: "smooth",
  });
};

export const scrollToTop = async (section, setOldAnketer) => {

  await document.querySelectorAll(section)[0].scrollIntoView({
    behavior: "smooth",
  });
};
...
export const animateOnLoad = async (
  ...
  await animate(
    ref,
    { scale: [0.8, 1], filter: ["blur(2px)", "blur(0px)"] },
    { duration: 1 }
  );
};

export const animateOnExit = async (
  ref,
  changePosition = "default",
  time = 1
) => {
  await animate(
    ref,
    { scale: [1, 0.8], filter: ["blur(0px)", "blur(2px)"] },
    { duration: time }
  );
  switch (changePosition) {
    case "top":
